...
class PriceAlert(db.Model):
    ...
    auto_disable = Column(Boolean, default=True, nullable=False)  # Disable after trigger
    ...
    def __repr__(self):
        return f'<PriceAlert {self.id}: {self.ticker} {self.alert_type} {self.threshold_price or self.threshold_percent}>'
    ...
    def check_trigger(self, current_price):
        ...
        if self.alert_type == 'above' and current_price >= self.threshold_price:
            triggered = True
        elif self.alert_type == 'below' and current_price <= self.threshold_price:
            triggered = True
        elif self.alert_type == 'change_percent' and self.last_price:
