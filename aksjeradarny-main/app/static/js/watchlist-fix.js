...
class WatchlistStateManager {
    ...
    overrideRefreshFunctions() {
        ...
        // Override any setTimeout-based refresh loops
        const originalSetTimeout = window.setTimeout;
        window.setTimeout = (callback, delay, ...args) => {
            // Check if this is a refresh-related timeout
            if (typeof callback === 'function') {
                const callbackStr = callback.toString();
                if ((callbackStr.includes('reload') || callbackStr.includes('location.href')) && 
                    delay < 30000) { // Less than 30 seconds
                    console.log('WatchlistStateManager: Intercepting rapid refresh timeout');
                    // Replace with our safe reload
                    return originalSetTimeout(() => {
                        window.safeReload();
                    }, Math.max(delay, this.updateCooldown), ...args);
                }
            }
