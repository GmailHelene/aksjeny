...
def access_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Use unified trial logic first for unauthenticated users
        if not current_user.is_authenticated:
            trial_status = _check_trial_access()
            logging.warning(f"[ACCESS_REQUIRED] trial_status={trial_status}")
            if trial_status.get('active'):
                logging.warning("[ACCESS_REQUIRED] Trial active, allowing access.")
                return f(*args, **kwargs)
            # If trial is expired or not started, redirect to /demo directly
            if trial_status.get('expired') or trial_status.get('no_trial_started') or not trial_status.get('active'):
            ...
            from flask import Response, url_for
            location = url_for('main.demo', source='trial_expired')
            logging.warning(f"[ACCESS_REQUIRED] Fallback, redirecting to {location}")
            return Response('', status=302, headers={'Location': location})

        # For authenticated users, now safe to access current_user properties
        ...
        if trial_status.get('active'):
            return f(*args, **kwargs)
        # No valid access for authenticated user
...
def _handle_no_access():
    ...
    try:
        ...
        return "Trial expired - please subscribe", 403
