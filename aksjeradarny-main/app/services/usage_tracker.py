...
class UsageTracker:
    ...
    def track_analysis_request(ticker=None):
        """Track when a user requests an analysis"""
        usage_key = UsageTracker._get_usage_key('daily_analyses')
        today = date.today().isoformat()
        
        # Get current usage
        current_usage = session.get(usage_key, {})
        if not isinstance(current_usage, dict):
            current_usage = {}
        
        # Reset if it's a new day
        if current_usage.get('date') != today:
            current_usage = {'date': today, 'count': 0}
        
        # Increment counter
        current_usage['count'] += 1
        session[usage_key] = current_usage
    ...
    def get_daily_analysis_usage():
        """Get current daily analysis usage count"""
        usage_key = UsageTracker._get_usage_key('daily_analyses')
        today = date.today().isoformat()
        
        current_usage = session.get(usage_key, {})
        if not isinstance(current_usage, dict) or current_usage.get('date') != today:
            return 0
        
        return current_usage.get('count', 0)
    ...
    def can_make_analysis_request():
        ...
        # Check current usage
        current_usage = UsageTracker.get_daily_analysis_usage()
        remaining = max(0, daily_limit - current_usage)
        
        return current_usage < daily_limit, daily_limit, remaining
