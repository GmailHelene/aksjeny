...
def api_search():
    ...
    try:
        ...
        current_app.logger.error(f"Error in stock search API: {str(e)}")
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            if not current_app.config.get('MAIL_USERNAME') or not current_app.config.get('MAIL_DEFAULT_SENDER'):
                current_app.logger.error("Mail not configured. MAIL_USERNAME or MAIL_DEFAULT_SENDER missing.")
                flash('E-post er ikke konfigurert på serveren. Kontakt administrator.', 'danger')
            ...
            try:
                ...
                flash(f'Kunne ikke sende e-post. Kontakt support på helene721@gmail.com. Feil: {str(e)}', 'danger')
        else:
